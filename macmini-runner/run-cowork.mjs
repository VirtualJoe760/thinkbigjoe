...
function buildPrompt(job) {
  ...
  return [
    ...
    `3. ${job.intent === "find_leads" ? "Source matching owners/principals (Sales Nav recipe in the prospecting memo, or public web), run deep recon → fit score → draft a humble connection note, and INSERT new rows into the `prospects` + `outreach` tables (status draft) via the DATABASE_URL in .env.local. Skip anyone already in the DB. For each prospect, capture their company website, visible LinkedIn profile photo URL, website status, 1-10 website rating, website notes, and 2-5 sales opportunities from deeper research inside prospects.recon using keys: websiteUrl, photoUrl, websiteStatus, websiteRating, websiteNotes, salesOpportunities." : "Send the approved/queued opening notes for qualified prospects."}`,
    `4. ${mode}`,
