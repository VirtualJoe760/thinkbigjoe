...
| **0.4** | **Shared UI primitives** — `Card/Field/Badge/StatusPill` in `src/components/ui/` (button.tsx recipe: lookup tables + `*Class()` helper + component) | med | `ui-primitives` plugin + future portal/admin plugins | 🟡 core shipped `3226797` (Card/Field/Badge + 1 proof migration). Fan-out across CRM-dense files deferred pending API sign-off. Second wave (`Stat`, `SectionHeading`) not started. |
