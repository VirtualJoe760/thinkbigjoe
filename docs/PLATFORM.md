...
| Step | What | Risk | Serves |
|---|---|---|---|
| **0.1** | **This roadmap doc** (+ index entry) | none | anchors the effort |
| **0.2** | **Consolidate the marketing surface** — move the bespoke marketing components into a `src/components/marketing/` module; tidy the flat `src/components/` dir | low (low import fan-out) | Phase 1 (clean unit to extract) + the cleanup Joe asked for |
| **0.3** | **Prune dead code** — zero-importer components (`pricing.tsx`, `contact-status.tsx` — verify), retired top-level dirs (`linkedin-sender/`, `windows-sender/`, `vps-sentinel/`, `macmini-runner/`), deprecated `command/sites`. **Code only — DB schema pruning is separate + risky (DB is source of truth).** | low–med | shrinks surface before the big work |
| **0.4** | **Shared UI primitives** — `Card/Panel/Badge/Field/StatusPill` in `src/components/ui/`, adopted incrementally (follow the `<AppHeader>` one-shell/thin-config model) | med | `ui-primitives` plugin + future portal/admin plugins |
| **0.5** | **Route-group separation** — split `(frontend)` into `(marketing)` / `(portal)` / `(command)` so the tree reflects the plugin boundaries (URLs unchanged) | med (touches many imports) | every later phase |
