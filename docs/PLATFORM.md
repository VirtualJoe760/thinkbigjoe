...
| **0.3b** | **Prune dead exports** — 8 verified-dead lib exports (tsc-gated). `analysisToBusinessJson` (site-analyzer.ts) *kept* — commented "for a future send to forge," staged for this platform | low | shrinks surface | ✅ done (removed 8; kept the staged one) |
...
| **0.4** | **Shared UI primitives** — `Card/Field/Badge/StatusPill/StatTile/SectionHeading` in `src/components/ui/` (button.tsx recipe: lookup tables + `*Class()` helper + component) | med | `ui-primitives` plugin + future portal/admin plugins | ✅ **done.** 7 primitives + barrel (`3226797`, `2fbdc08`); command fan-out (`5bcffd0`, 9 surfaces); StatusPill consolidation (`735a105`); customer-facing fan-out (`44a4787`: leads-crm + 7 portal + 3 marketing + booking-wizard). Every swap verified className-identical (StatusPill the one sanctioned exception), green `pnpm build`, live render checks. ~21 files now on the primitives. |
...
**StatusPill consolidation (done for the clean maps):** 3 ad-hoc status-color maps now render via the
canonical `StatusPill` (sites-queue site-status, outreach-card `BADGE`, design-reports `STATUS_STYLE`)
— a deliberate, approved visual normalization (canonical `-50/-700` shade + standard pill size). Left
alone on purpose: sites-queue `OUTREACH_PILL` (its violet "replied" state has no canonical tone —
collapsing it loses meaning, not just shade) and `leads-crm.tsx` (blocked on Joe's WIP commit).
