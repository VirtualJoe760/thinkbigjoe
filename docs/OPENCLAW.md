...
| `destiny` | gigs | **Joe's Upwork contract hunter** (📄), built 2026-08-30. Reads the gigs Upwork emails him, scores each on **fit** and — the number that actually decides — **winnability with an empty profile**, posts them to **`/command/gigs`** for approval, and drafts the proposal. **Joe submits; she never does.** That is not a workflow preference: Upwork permanently bans accounts (no appeal) for auto-submitting proposals, scraping the job feed, or letting a tool log in as you. Her hard blocks say she never logs into Upwork, never scrapes it, never submits, and never spends a Connect — a proposal costs 14–25 Connects (~$2–4), so a bad bid is real money. Ingestion is Upwork's own alert email into the `Upwork` folder, because RSS was retired in Aug 2024 (bots auto-bidding on it) and the API is partner-gated. Consequence baked into her rubric: alerts run **15–60 min behind**, so she must never chase gigs decided by speed or price — she targets thin-supply niche agent work instead. Model `claude-cli/claude-sonnet-4-6`. **Ships COLD** — cron `enabled:false` until Joe sets up saved-search alerts + the mail filter. Tools: `list_gig_alerts`, `record_found_gig`, `list_approved_gigs`, `save_gig_proposal`, `update_gig_status`, `list_my_directives`, `complete_directive`, `log_activity`. |
