...
| **3b** | **Forge deterministic theme fast-path** | M | `edit-poll.mjs`: a **token-only batch skips `claude -p`** — deterministically write a fenced `TBJ-THEME` `:root` block at the end of the site's `globals.css` (from the `edits` jsonb) + wire the `next/font` import, then build+deploy. Mixed/free-text batches still route to the LLM. Cheaper, faster, and can't misfire. |
