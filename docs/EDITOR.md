...
- **Notched-phone safe-area:** ~~two stacked bottom bars on the Site tab can double-consume the inset~~
  — resolved: the portal's app tab bar (`portal-bottom-nav.tsx`) was **removed** when the nav was
  unified to `<AppHeader>` (see VENUS_UI_MAPPING.md "Nav chrome"), so the Site editor's own bottom bar
