...
| `scripts/vm-text-fallback.sh` → `POST /api/leads/vm-text-fallback` | `com.thinkbigjoe.vmtextfallback` | Fires **every ~5 min**; safety net for the **delivery-gated voicemail follow-up text**. Normally the Drop Cowboy webhook sends the text on delivered/failed; this texts any lead still `vm_text_pending` ~8+ min after the drop (no webhook ever arrived) with the non-VM variant. See [VOICEMAIL.md](VOICEMAIL.md). |
