...
| **Destiny** | `/command/gigs` | **Joe's Upwork gig board** — the `destiny` agent scores gigs into the `gigs` table (status `found`); Joe **Approves/Dismisses** (`approveGig`/`dismissGig` in `gigs/actions.ts`), and she drafts a proposal for each approved one. Cards lead with **`win_score`, not `fit_score`** — Joe's profile has no reviews/JSS and his alerts run 15–60 min late, so "can he WIN it" outranks "can he do it". ⚠️ **`markSubmitted` is human-only by design** and the `update_gig_status` MCP tool *refuses* the `submitted` status: Upwork permanently bans accounts for automated bidding, so Joe clicking submit himself is the compliance boundary, not a UX preference. Stages: `found → approved/dismissed → drafted → submitted → won/lost`. See OPENCLAW.md's `destiny` row. **Ships cold.** |
...
| **TBJ Destiny — Upwork Gigs** | destiny | **🧊 SHIPS COLD** (`enabled:false`; `0 8,16 * * *` Phoenix when on) | `list_gig_alerts`, `record_found_gig`, `list_approved_gigs`, `save_gig_proposal`, `update_gig_status`, `log_activity` | `/command/gigs`. Reads **Upwork's own saved-search alert emails** from the `Upwork` mail folder — the ONLY compliant ingestion path (Upwork bans accounts for scraping the feed *or* auto-bidding; RSS was killed Aug 2024 for exactly that, and the official API is gated behind partner approval). Scores each posting on `fit_score` **and** `win_score`, posts the good ones for Joe's approval, drafts proposals for approved ones, caps at **3 proposals/run**. **Never logs into Upwork, never submits, never spends a Connect.** ⚠️ **One** setup step gates turning it on: Joe creates the saved-search alerts in Upwork's own UI — nobody can do that for him. A mail filter into the `Upwork` folder is optional tidiness, because `list_gig_alerts` also sweeps INBOX for `upwork.com` mail (requiring a filter would make "no gigs" and "filter missing" look identical — a silent failure costing a model call per run). |
