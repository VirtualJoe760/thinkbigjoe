...
| TBJ Venus — Job Hunt Debrief | **main (Venus)** | **✅ LIVE** — `30 12,19 * * *` America/Phoenix (12:30 + 19:30) | `get_job_hunt_report`, `send_telegram_update` | Joe's **Telegram**, sourced from `/command/applications`. What Whitney applied to **by title + company**, interview-stage roles, and every question still pending on Joe. Numbers come from the tables, not Whitney's self-reported notes. ⚠️ Two things about this row are load-bearing — see **Cron delivery** below: `agent: "main"` (NOT agentless) and an explicit `channel`+`to`. |
| TBJ Whitney — Job Applications | whitney | **✅ LIVE** (`enabled:true`, `*/15 * * * *` — every 15 min, 24/7; stagger 2m. Her apply-gate is now open: target profile, `RESUME_PATH`, `LINKEDIN_URL`, `JOB_SIGNUP_PASSWORD` are all set) | `list_approved_jobs`, `update_application_status`, `inbox_search`, `record_found_job`, `book_appointment`, `record_question`, `list_answered_questions`, `mark_question_resolved` | `/command/applications`. **Priority-queue**: applies to the top approved job (create account → email-verify → tailor → submit), and only *fills* by finding new roles when the approved queue is empty. **Her cron has no delivery channel on purpose** — she never messages Joe from the cron. Escalation rides the MCP tool instead: `record_question` pings Joe's Telegram directly (see the Whitney row above). |
| TBJ Edward — Inbox Sweep | edward | **⏸ DISABLED** (ships cold with Edward; 5:45a/11:45a/5:45p Phoenix when enabled) | `inbox_sweep`, `email_create_draft`, `email_move_spam`, `email_request_send`, `email_list_pending_sends`, `log_activity` | `/command/inbox`. Triage joe@thinkbigjoe.com: classify, junk spam (never delete), draft replies in Joe's voice, queue sends for Venus, file the `email_inbox_report` she briefs from. |
| TBJ Venus — Inbox Update | *(main, system-event)* | **⏸ DISABLED** (ships cold with Edward; 6:00/12:00/18:00 Phoenix when enabled) — **delivers to Joe's Telegram** (`channel`/`to` fields in the manifest; the sync script passes `--announce --channel --to`) | `get_inbox_report`, `email_list_pending_sends`, `email_approve_send`, `email_reject_send` | Venus reads Edward's report, **decides every pending send**, and messages Joe the morning/afternoon/evening inbox update (🔴 pressing · 📥 inbox · 📤 decided · 📝 waiting). Inbox-scoped for now; widen to full org rollup later. |
