...
| **Quick Money** | `/command/money` + `/command/money/[id]` | **The money desk — Max 💸 + Ryan 🔍**, the pair hunting cash Joe can make *this week* (they replaced `researcher`, retired 2026-09-03). Shows what each is on right now (the claim lock means never the same idea), their **verdicts**, their **conversation**, and the graveyard of ruled-out ideas. `/command/money/[id]` renders the `claude -p`-written HTML report in a **`sandbox=""` iframe** — model-generated markup in a privileged admin app is not something to `dangerouslySetInnerHTML`. ⚠️ **This board is a MIRROR, not the master.** Their real state is a JSON file on Joe's Mac (`~/.openclaw/shared/max-ryan/desk.json`) because the claim lock, the 3-round cap and the **7-day ASAP bar** are enforced inside `desk.mjs` — that is what makes "they never work the same idea at once" a guarantee rather than a prompt instruction. Vercel can't read that file, so **`scripts/money-desk-sync.mjs`** (launchd `com.thinkbigjoe.moneydesksync`, every 5 min, log `/tmp/tbj-money-desk-sync.log`) pushes it into `money_desk_state` / `money_desk_messages` / `money_desk_verdicts`. **Nothing here writes back** — a write would be clobbered on the next pass *and* desync the lock the agents depend on. The page warns when the mirror is >20min stale, because a stopped sync and a quiet desk look identical otherwise. **Dissent is rendered louder than agreement, on purpose:** these two are built as opposites because identical agents don't validate each other, they agree with each other — so a verdict with **no** recorded objection is flagged in red as *unreviewed*, not shown as a clean win. The list query deliberately omits `report_html` (~9KB/row; egress). **Ships cold** — the banner tells you so and how to switch them on. See OPENCLAW.md's `max`+`ryan` row. |
| **Destiny** | `/command/gigs` + `/command/gigs/[id]` | **Joe's Upwork gig board** — rebuilt 2026-08-31 to mirror Whitney's `/command/applications` (same shape of work, so it reads the same): pause/resume, count chips, `DirectAgent`, an **open-questions** section, a sortable + paginated review board, an in-flight pipeline, outcomes, and an archive. Shares `parts.tsx` presentational pieces with a **full-gig detail page** at `/command/gigs/[id]` (posting, her two reads, client signals, the proposal she sent, timeline stamps). Destiny hunts the live Upwork feed in a browser and scores gigs into `gigs` (status `found`); Joe **Approves/Dismisses**, and on approval **she writes the proposal and submits it herself**. Cards lead with **`win_score`, not `fit_score`** — no reviews/JSS and she now spends the Connects, so "can he WIN it" outranks "can he do it"; `ClientSignals` flags a crowded posting (20+ bids) or an unverified/never-hired client in red. **Board capped at 10 `found`** (`REVIEW_CAP` here mirrors `GIG_REVIEW_CAP` in tbj-mcp.mjs) — a full board renders an explicit "she has stopped hunting" banner, so quiet reads as standing down rather than broken. ⚠️ `markSubmitted` is a **manual override** (she sets `submitted` via `update_gig_status`, rate-limited 5/day soft + 45 min apart, note required). A gig at `drafted` gets its OWN section — it means something STOPPED her mid-submit. **Pause is the emergency brake**: `setDestinyPaused` writes `agents.paused`, which her loop-entry tools read and stand down on. **No duplicate bids:** `gigs.url_key` (Upwork's `~0` job id, UNIQUE) means one posting = one row regardless of link decoration, and `update_gig_status` refuses a re-submit or an unapproved submit. Stages: `found → approved/dismissed → drafted → submitted → won/lost`. **Ships cold.** |
...
| **Vera** | `/command/investors` | **The ChatRealty angel-investor pipeline** — and the one board in the Command Center whose data belongs to a **different company** (ChatRealty is org #2; everything else is TBJ org #1). Vera (`angel-scout`) researches and qualifies angels against six axes (stage · check size · thesis · geography · **a verified check in the last 12 months** · portfolio conflicts), and records them to `investors` via `add_investor`. **Every record shows its source URLs at the point of decision** — that is the whole design: the documented way an LLM fails at investor research is *fabricating* plausible funds, partners and email addresses, and the cost lands two hops downstream when Edward drafts a confident email on top and Joe sends it. The write path enforces it rather than trusting the prompt: `add_investor` **refuses** a record with no sources, an email with no `email_source`, a `qualified` record with no `why_fit` line, or a disqualification with no reason. Cards flag a stale last-check (>365d) and an unsourced email in red. Joe re-tiers (T1/T2/T3), marks Passed, or **Cuts with a reason** (the reason is what stops Vera re-adding them next month); `setVeraPaused` → `agents.paused` is the emergency brake her tools read. Dedupe is on `dedupe_key` (LinkedIn URL, falling back to name+firm) — UNIQUE per org, the constraint that stops Edward emailing one investor twice. Handoff: `list_investors_for_outreach` is **Edward's** read, and returns either a sourced address or an explicit "none on file — warm intro only". **No send button on this page, by design.** Stages: `qualified → drafting → awaiting_approval → contacted → replied` (or `passed`/`disqualified`). **Ships cold.** See OPENCLAW.md's `angel-scout` row.
...
| **TBJ Destiny — Upwork Gigs** | destiny | **🧊 SHIPS COLD** (`enabled:false`; `0 8,16 * * *` Phoenix when on — twice a day, since she submits one proposal per run and caps at 5/day, so extra wake-ups only cost Joe's shared Claude budget) | `gig_board_count`, `record_found_gig`, `list_approved_gigs`, `save_gig_proposal`, `update_gig_status`, `record_question`, `list_answered_questions`, `mark_question_resolved`, `get_candidate_facts`, `remember_fact`, `list_my_directives`, `complete_directive`, `log_activity` | `/command/gigs`. **Priority queue:** first submit a proposal for a gig Joe approved; then read the Upwork inbox for **invitations** (highest-win-rate work there is — no Connects, no competing bids) and client replies; only as filler, hunt the feed and post scored gigs for approval. **Calls `gig_board_count` before hunting, always** — a full board (10) means stand down without browsing, the lesson that cost Whitney an entire Ollama quota. ⚠️ **One setup step gates turning it on:** Joe signs into Upwork once in the browser profile she drives — she never authenticates (no credentials, no 2FA), so without a live session every run dies at a login page. The old `list_gig_alerts` email path was deleted with the 2026-08-31 rewrite. |

> ⚠️ **`agent_questions` is shared by TWO boards since 2026-08-31.** The row carries `agent` plus at most one of `application_id` (Whitney → `/command/applications`) or the new `gig_id` (Destiny → `/command/gigs`). Both pages filter on `agent`, and `record_question` / `list_answered_questions` / `mark_question_resolved` are agent-scoped — they default to `whitney`, so Destiny MUST pass `agent:"destiny"`. `mark_question_resolved` refuses a question belonging to another agent, because resolving someone else's silently discards Joe's answer. Declining is a DECISION on both boards: it cancels the application (Whitney) or dismisses the gig (Destiny).
| **TBJ Edward — Application Follow-up** | edward | **✅ LIVE** — `30 11 * * 1-5` America/Phoenix (11:30, **weekdays only**, deliberately 1h before Venus's 12:30 debrief so she always reads a fresh report) | `list_followup_due`, `inbox_sweep`, `inbox_unanswered`, `record_employer_reply`, `email_create_draft`, `email_request_send`, `email_file`, `log_activity` | `/command/applications` + `/command/inbox`. **The sweep cron asks "what arrived"; this one asks "what never came back."** An application that is submitted, auto-confirmed, then ignored forever is invisible because **`applied` looks like success** — `list_followup_due` surfaces anything silent 7+ days with no employer reply and no prior nudge. Edward drafts one short follow-up each (into the existing ATS thread in `Job Alerts`, addressed to **Reply-To**), caps at 5/run, queues them for Venus, and files a `job_followup_report`. `get_inbox_report` returns that report alongside the sweep, so Venus's 🔁 **FOLLOW-UPS** section reaches Joe on Telegram. **Applications with no reachable human are reported, not faked** — only Joe can chase those. |
...
| **ChatRealty Vera — Investor Research** | angel-scout | **🧊 SHIPS COLD** (`enabled:false`; `0 10 * * 2,5` Phoenix when on — Tue + Fri, twice a week, because verification is slow work and the pipeline only moves as fast as Joe can hold conversations) | `list_my_directives`, `complete_directive`, `list_investors`, `add_investor`, `update_investor`, `log_activity`, `send_telegram_update` | `/command/investors`. Researches angels for the **ChatRealty** raise — a different company from TBJ. **Calls `list_investors` before searching, always**, so the same person is never researched twice. Works primary sources first (SEC EDGAR Form D, the investor's own dated portfolio page) and treats a "top angels" listicle as a source of *names*, never of *facts*. **Capped at 10 fully verified investors per run** — she is on `claude-cli/claude-sonnet-4-6` (chosen deliberately: fabrication is the known failure mode of this job and the cheap seat is the wrong economy), so every run draws the **shared Claude Max weekly cap**. Don't raise the cap to get more names — a long list *is* the failure mode. She has **no send path of any kind**: she hands bios to Edward, Venus approves, Joe sends.
