...
| [`NEWSLETTER.md`](NEWSLETTER.md) | You're touching the **$99-plan monthly customer newsletter** — the portal surface (`/portal/newsletter`), the AI draft/render/send engine (`src/lib/newsletter.ts`), the `newsletter_contacts` / `newsletters` tables, `sendNewsletterEmail`, or the unsubscribe route. Client uploads a list → AI drafts → they approve + send. |
