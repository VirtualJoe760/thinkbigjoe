...
| [`DIALER.md`](DIALER.md) | You're touching **Joe's call-session dialer** (/command/dialer), the recording drop-box (`/api/dialer/recording`), or the `dial_call`/`dial_recording` timeline events. Boost-phone-native calling (no Twilio in the call path), one-tap dispositions, FolderSync recording upload → Blob → Gemini AI notes feeding `contact_notes` + the outreach agent. Phone setup steps + the recording-consent rule live here. |
| [`COLD_EMAIL.md`](COLD_EMAIL.md) | You're writing or changing **any cold outreach copy or its senders** (email first-touch, the outreach agent's email play, SMS first-touch framing). The **personal-first doctrine** (2026-07-26): no cold claim-code sends — the Kyle-email model + Hormozi's rules (personalize→big fast value, sub-3rd-grade language, one soft CTA, multi-channel follow-up, ACA on replies), the TBJ first-touch shape + examples, and the implementation map (claim-code send = manual leads-dashboard action). |
...
| [`NANO_CREW.md`](NANO_CREW.md) | **You're working on funding for Nano Crew** — the business/technical/investment-strategy dossier for the planned full-time **funding agent** (grants → credits → accelerators → angels → pre-seed, in that order), its hard rules (draft-only, never send/sign/overstate), and the build note (THE RULE applies when the agent ships). ⚠️ Nano Crew itself is a **separate codebase** (`~/code/nanocrew`) — this doc grounds the pipeline, it does not authorize edits there. Numbers verified against that repo 2026-08-24; re-verify before pitching. |
...
| [`/mcp-server/research/README.md`](../mcp-server/research/README.md) | Active (added 2026-07-30) | **Not part of the TBJ pipeline.** Two standalone MCP servers built for a research client, deployed via **Cowork** and designed to run unattended for days: `research-mcp` (indexing layer → stratified reading → PRISMA/GRADE white paper + self-contained visual report + versioned JSON dataset; sources are Google/DuckDuckGo/Yandex/Baidu plus PubMed, Europe PMC, ClinicalTrials.gov, OpenAlex, Crossref, openFDA) and `thesis-mcp` (investigational thesis builder where every parameter must trace to a recorded finding). Shares no code, DB or config with `tbj-mcp.mjs`; corpus is snapshot+tail JSONL outside the repo. THE RULE (UI + tool + cron) does not apply — there is no TBJ surface for it. Its own durability contract (compaction, persistent quotas, heartbeat) is documented in its README under "Surviving a multi-day run" — **read that before changing anything in `store.mjs` or `ratelimit.mjs`**. |
