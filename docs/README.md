...
| [`AUTH.md`](AUTH.md) | You're touching **login, the admin gate, transactional email, or the domain/DNS/email setup** — better-auth wiring, the admin allowlist, the Google-only-account/no-password gotcha, the Zoho SMTP setup, `/api/health/email`, **and the authoritative "where everything lives" map: DNS = Vercel, email = Zoho, plus deliverability (SPF/DKIM/DMARC) status.** Check it before asking where DNS or email lives. |
